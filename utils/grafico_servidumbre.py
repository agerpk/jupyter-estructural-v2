...
def graficar_servidumbre(estructura_geometria, servidumbre_obj, usar_plotly=True, metodo="AEA"):
    """
    Genera gráfico de estructura con zonas de servidumbre superpuestas
    
    Args:
        estructura_geometria: Objeto EstructuraAEA_Geometria
        servidumbre_obj: Objeto ServidumbreAEA o ServidumbreAyEE con cálculos
        usar_plotly: Si True usa Plotly, si False usa Matplotlib
        metodo: Método usado ("AEA" o "AyEE")
    ...
    # Título según método
    if metodo == "AyEE":
        titulo_metodo = "FRANJA DE SERVIDUMBRE AyEE"
    else:
        titulo_metodo = "FRANJA DE SERVIDUMBRE AEA-95301-2007"
    
    # Configurar layout
    fig.update_layout(
        title=dict(
            text=f"{titulo_metodo}<br>{estructura_geometria.tension_nominal}kV - {estructura_geometria.tipo_estructura.upper()}",
